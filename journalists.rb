# méthode afin de simplifier l'affichage sur terminal en ajoutant une pause après chaque ajout de données
def continue
  puts
...
# affiche le nombre d'éléments dans l'array journalistes
def nombreliste
  puts
  puts "Le nombre de journalistes de la liste est de #{@journalistes.length}"
  continue
end
...
def aude
  ...
  puts "Le nombre de journalistes dont le compte contient Aude est de #{nombreaude}"
  continue
end
...
def alphabetliste
  ...
  print @journalistes.sort_by { |sorter| sorter.downcase }
  puts
  10. times do print "-" end
  puts
  puts "FIN DE LA LISTE DES JOURNALISTES PAR ORDRE ALPHABETIQUE"
  continue
end
...
=begin
  Recette pour un bon punch cubain
    Prenez  une mesure de rhum ambré,
            une mesure de jus d'orange,
            une mesure de jus d'ananas,
    Toppez avec de la limonade,
    Ajoutez du sirop de grenadine,
